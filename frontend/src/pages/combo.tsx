...
const Combos = () => {
  ...
  const handleAddCombo = (combo: typeof combos[0]) => {
    ...
    addItem({
      id: combo.id, // id especial
      name: combo.name,
      brand: "Combo tecnológico",
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {combos.map((combo, i) => (
              ...
              >
                ...
                <div className="absolute top-4 right-4 flex items-center gap-1 px-3 py-1 rounded-full bg-gray-500/70 text-white text-sm font-bold">
                  <Percent size={14} />
                  {combo.discount}% OFF
                </div>
                ...
                <div className="flex gap-2 mb-6">
                  {combo.images.map((img, idx) => (
                    <div
                      key={idx}
                      className="w-20 h-20 rounded-xl bg-muted/30 overflow-hidden"
                    >
                      <img
                        src={img}
                        alt=""
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-primary/30 text-primary font-semibold hover:bg-primary hover:text-primary-foreground transition-colors"
                >
                  Agregar combo
                  <ArrowRight size={16} />
                </button>
